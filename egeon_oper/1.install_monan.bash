...
# - DE: files generated by the compilation breaks the git checkout, due to merge conflicts. Probabily is needed the deletion of these files
...
export MONANDIR=${MONAN_SRC_DIR}/MONAN-Model_${tag_or_branch_name}_egeon.gnu940
...
if git checkout "$tag_or_branch_name" 2>/dev/null; then
    git pull
    echo -e "${GREEN}==>${NC} Successfully checked out and updated branch: $tag_or_branch_name"
else
    echo -e "${RED}==>${NC} Failed to check out branch: $tag_or_branch_name"
